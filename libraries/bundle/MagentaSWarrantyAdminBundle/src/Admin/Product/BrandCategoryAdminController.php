...
class BrandCategoryAdminController extends BaseCRUDAdminController {
	...
	public function crudAction($id = null, $operation, Request $request) {
		...
		if($operation !== 'CREATE') {
			...
			/** @var BrandCategory $cat */
			$cat = $this->admin->getObject($id);
			if( ! $cat) {
				throw $this->createNotFoundException(sprintf('unable to find the Category with id: %s', $id));
			}
			if($operation === 'UPDATE') {
